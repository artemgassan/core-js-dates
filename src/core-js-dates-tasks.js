...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const millisecondsDays = 24 * 60 * 60 * 1000;
  ...
  const diffMs = Math.abs(endMidnight - startMidnight);
  const diffDays = diffMs / millisecondsDays;
...
function isDateInPeriod(date, period) {
  ...
  const min = Math.min(startUTC, endUTC);
  const max = Math.max(startUTC, endUTC);
  return inputUTC >= min && inputUTC <= max;
}
...
function getWeekNumberByDate(date) {
  ...
  const weekNumber = Math.ceil(
    ((temp.getTime() - yearStart.getTime()) / millisecondsDays + 1) / 7
  );
